...
- **상세:**
  - v6.0.0 안정 주행 흐름은 유지하면서, 보관함 주차 중 fresh wall을 끝까지 잡지 못하는 테스트/비정상 환경을 대비해 145초 이후 최후수단 패턴을 추가했다.
  - 최후수단은 시작 직후 5cm 후진, 현재 헤딩 기준 왼쪽 95도 회전, 5초 고속 후진, 이후 오른쪽 횡이동과 후진을 2초씩 반복하고 12초 경과 시 강제 덤프 단계로 넘어간다.
  - pose 기반 접촉 fallback은 최후수단과 겹쳐 실제 원인을 흐릴 수 있어 `storage_pose_fallback_enabled: false`로 비활성화했다. 관련 파라미터와 코드는 남겨 두어 필요 시 설정 하나로 재활성화할 수 있다.
  - 최신 run 분석에서 최후수단의 5초 후진은 아직 실행되지 않았고, 기존 `REVERSE TO LEFT WALL` 단계의 후진이 길게 지속된 것이 확인되어 `storage_last_chance_reverse_sec: 5.0`은 유지했다.
  - 현재 예선1 설정은 사용자 요청 상태 그대로 Set1 `cube`, Set2 `apple`을 사용한다.
  - `test_timed_storage_route.py` 회귀 테스트 17개를 통과했고, `robot_planning`·`robot_bringup` 빌드를 완료했다.
- **변경 파일:**
  - `PROJECT_LOG.md` / 수정
  - `ros2_ws/src/robot_bringup/config/motion_tuning.yaml` / 수정
  - `ros2_ws/src/robot_planning/robot_planning/nodes/mission_fsm_node.py` / 수정
  - `ros2_ws/src/robot_planning/test/test_timed_storage_route.py` / 수정
- **다음 할 일 반영:** 예선1용 보관함 최후수단 fallback 설정·테스트·빌드 항목을 완료 처리하고, 실제 경기장에서 fresh wall/최후수단/덤프 완료 시각을 확인하는 항목을 추가했다.
- **버전 근거:** 보관함 복귀 실패 대비 최후수단 상태기계와 설정 파라미터를 추가한 기능 변경이므로 MINOR를 `v6.0.0`에서 `v6.1.0`으로 올렸다.
